...
export class GeminiProvider extends BaseAIProvider {
  ...
  private getGenerativeModel(options: {
    model: string
    ...
    if (/^gemini-3\.(?:5|6)-/.test(options.model)) {
      delete generationConfig.temperature
    }

    return {
      generateContent: async (contents: string) => {
  ...
  async validateConfig(): Promise<boolean> {
    try {
      const model = this.getGenerativeModel({ model: this.config.model, json: false })
...
Only return valid JSON, no other text.`
  ...
  estimateCost(operation: string, inputLength: number): CostEstimate {
    ...
    const costPerMillionTokens = this.averageCostPerMillion()
  ...
  protected calculateCost(tokensUsed: number): number {
    return (tokensUsed / 1000000) * this.averageCostPerMillion()
  }

  private averageCostPerMillion(): number {
    const rates: Record<string, number> = {
      'gemini-3.6-flash': 4.5,
      'gemini-3.5-flash-lite': 1.4,
      'gemini-2.5-flash': 1.4,
    }
    return rates[this.config.model] ?? 3
  }
