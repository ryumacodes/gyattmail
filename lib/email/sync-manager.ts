...
import { listAgentMailMessages } from './agentmail-client'
...
export async function syncAccount(
  ...
  if (account.provider === 'agentmail') {
    try {
      onProgress?.({ accountId: account.id, folder: 'INBOX', status: 'syncing', message: 'Refreshing AgentMail inbox…' })
      ...
      await updateAccountStatus(account.id, 'connected')
      onProgress?.({ accountId: account.id, folder: 'INBOX', status: 'completed', message: 'AgentMail is up to date', newEmails: messages.length, totalEmails: messages.length })
      return [{ accountId: account.id, folder: 'INBOX', newEmails: messages.length, totalEmails: messages.length }]
    } catch (error) {
      const message = error instanceof Error ? error.message : 'AgentMail sync failed'
      await updateAccountStatus(account.id, 'failed', message)
      return [{ accountId: account.id, folder: 'INBOX', newEmails: 0, totalEmails: 0, error: message }]
    }
  }
