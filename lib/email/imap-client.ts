...
export async function updateMessageLabels(
  ...
  try {
    await client.mailboxOpen(folder)
    if (addLabels.length) await client.messageFlagsAdd(uid, addLabels, { uid: true })
    if (removeLabels.length) await client.messageFlagsRemove(uid, removeLabels, { uid: true })
  } finally {
    await client.logout()
  }
}
