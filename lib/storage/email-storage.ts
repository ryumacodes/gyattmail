...
export async function updateEmailMetadata(
  emailId: string,
  updates: Pick<Partial<EmailMessage>, 'labels' | 'snoozeUntil' | 'repliedTo'>
...
export async function removeStoredEmail(emailId: string): Promise<void> {
  const email = await getEmail(emailId)
  if (!email) return
  const emails = await loadEmails(email.accountId, email.folder)
  await saveEmails(email.accountId, email.folder, emails.filter((item) => item.id !== email.id))
}

export async function moveStoredEmail(emailId: string, targetFolder: string): Promise<EmailMessage | null> {
  const email = await getEmail(emailId)
  if (!email) return null
  await removeStoredEmail(emailId)
  const moved: EmailMessage = {
    ...email,
    folder: targetFolder,
    id: `${email.accountId}:${targetFolder}:${email.uid}`,
    syncedAt: new Date().toISOString(),
  }
  await addEmails(email.accountId, targetFolder, [moved])
  return moved
}

export async function loadAllEmails(accountId: string): Promise<EmailMessage[]> {
  ...
  try {
    ...
    const groups = await Promise.all(files
      ...
      .map(async (file) => loadEmails(accountId, file.replace('.json', '').replace(/_/g, '/'))))
    return groups.flat()
  } catch (error) {
