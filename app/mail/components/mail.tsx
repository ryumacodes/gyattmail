...
import type { SavedMailView } from '@/lib/types/mail-preferences'
import { MobileMailNavigation } from '@/app/mail/components/mobile-mail-navigation'
...
export function Mail({
  ...
  const [savedViews, setSavedViews] = React.useState<SavedMailView[]>([])
  ...
  React.useEffect(() => {
    void fetch('/api/mail/preferences')
      ...
      .then((result) => setSavedViews(result.preferences?.savedViews || []))
  }, [showSettings])

  const focusMobileSearch = () => {
    if (mail.selected) setMail({ ...mail, selected: null })
  ...
  const mails = allMails.filter(m => {
    const account = accounts.find((item) => item.id === m.accountId)
    if (selectedAccount === 'human' && (m.ownerKind || account?.ownerKind) === 'agent') return false
    if (selectedAccount === 'agent' && (m.ownerKind || account?.ownerKind) !== 'agent') return false
    if (!['all', 'human', 'agent'].includes(selectedAccount) && m.accountId !== selectedAccount) return false
    if (!matchesSearchQuery(m, searchQuery)) return false
    const now = new Date()
  ...
  }, [currentFolder, selectedAccount, searchQuery])
    ...
    <DndContext onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <TooltipProvider delayDuration={0}>
        ...
        onLayoutChanged={(layout) => {
          const sizes = [layout.navigation, layout.list, layout.reader]
            .filter((size): size is number => typeof size === "number")
          document.cookie = `react-resizable-panels:layout:mail=${JSON.stringify(
            sizes
          )}`
        }}
        className="h-full min-w-0 items-stretch overflow-hidden bg-background paper-grain"
      >
        <ResizablePanel
          id="navigation"
        ...
        <ResizablePanel
          ...
          maxSize="45%"
          className={cn(
            "min-w-0 bg-background max-md:!flex-[1_1_100%] max-md:!max-w-full max-md:pb-[calc(3.5rem+env(safe-area-inset-bottom))]",
        ...
        >
          <Tabs defaultValue="all" className="h-full flex flex-col">
            <div className="flex-shrink-0">
              <div className="hatch-edge-bottom flex h-14 items-center gap-2 px-3 md:px-4">
                <button
                  type="button"
                  onClick={() => setShowMobileNavigation(true)}
                  className="flex size-10 shrink-0 items-center justify-center rounded-xl border-2 border-border bg-card md:hidden"
                  aria-label="Open mailboxes"
                >
                  <Menu className="size-5" />
                </button>
                <h1 className="min-w-0 truncate font-serif text-lg font-bold capitalize text-foreground md:text-xl">
                  {currentFolder}
                </h1>
                <TabsList className="ml-auto shrink-0">
                  <TabsTrigger value="all">
                    <span className="hidden min-[390px]:inline">All mail</span>
                ...
                <button
                  ...
                  className="ml-3 hidden p-2 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition-colors shadow-sm md:block"
                  aria-label="Compose"
                ...
                <button
                  ...
                  className="ml-2 hidden p-2 rounded-lg hover:bg-accent transition-colors text-muted-foreground hover:text-foreground md:block"
                  aria-label="Settings"
                >
              ...
              <div className="hatch-edge-bottom bg-background/95 p-3 backdrop-blur supports-[backdrop-filter]:bg-background/60 md:p-4">
                <form>
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="hatch-inset h-11 pl-8 md:h-9"
                    />
                  </div>
                </form>
                {savedViews.length > 0 && (
                  <div className="mt-2 flex gap-2 overflow-x-auto pb-1">
                    {savedViews.map((view) => (
                      <Button
                        key={view.id}
                        type="button"
                        variant={searchQuery === view.query ? 'default' : 'outline'}
                        size="sm"
                        className="shrink-0"
                        onClick={() => setSearchQuery(view.query)}
                      >
                        {view.name}
                      </Button>
                    ))}
            ...
            <TabsContent value="all" className={cn("m-0 flex-1 overflow-hidden flex flex-col data-[state=inactive]:hidden", mail.selectedIds.size > 0 && "pt-2")}>
              ...
              {/* Pagination Controls */}
              {totalPages > 1 && (
                <div className="flex-shrink-0 flex items-center justify-center gap-2 border-t bg-background px-2 py-2 md:justify-between md:px-4 md:py-3">
        ...
        {mail.selected && allMails.find((item) => item.id === mail.selected) && (
          <>
            ...
            <ResizablePanel
              ...
              maxSize="70%"
              className={cn(
                "min-w-0 bg-background paper-grain max-md:!flex-[1_1_100%] max-md:!max-w-full max-md:pb-[calc(3.5rem+env(safe-area-inset-bottom))]",
      ...
      <MobileMailNavigation
        open={showMobileNavigation}
        onOpenChange={setShowMobileNavigation}
        currentFolder={currentFolder}
        onFolderChange={setCurrentFolder}
        selectedAccount={selectedAccount}
        onAccountChange={setSelectedAccount}
