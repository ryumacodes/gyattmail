...
type PropagationEvent = Pick<React.SyntheticEvent, 'stopPropagation'>
...
interface DraggableMailRowProps {
  ...
  onItemClick: (mailId: string, index: number, event: React.MouseEvent | React.KeyboardEvent) => void
  onCheckboxClick: (mailId: string, index: number, e: React.MouseEvent) => void
  onReply: (event: PropagationEvent, mailId: string) => void
  onArchive: (event: PropagationEvent, mailId: string) => void
  onOpenLabelDialog: (event: PropagationEvent, mailId: string) => void
  onTrash: (event: PropagationEvent, mailId: string) => void
  onToggleRead: (event: PropagationEvent, mailId: string) => void
    ...
    <ContextMenu key={item.id}>
      <ContextMenuTrigger asChild>
        <div
          ...
          tabIndex={0}
          className={cn(
            "hatch-border flex min-h-24 items-start gap-2 rounded-[var(--radius)] p-2.5 text-left transition-all relative overflow-hidden cursor-pointer sm:gap-3 sm:p-3",
            isSelected && !isViewing && "hatch-border-active bg-accent shadow-sm",
            isViewing && "hatch-border-active bg-card shadow-letterpress",
            !isSelected && !isViewing && "bg-background hover:bg-accent",
          ...
          <div className="relative z-10 flex flex-col gap-2 flex-1 min-w-0">
            <div className="flex items-center">
              ...
              <div className="ml-auto flex min-w-0 items-center gap-1 sm:gap-2">
                <div
                  className={cn(
                    "max-w-20 truncate whitespace-nowrap sm:max-w-none",
                ...
                <button
                  ...
                  className={cn(
                    "flex size-10 flex-shrink-0 items-center justify-center transition-all duration-200 hover:scale-110 sm:size-auto",
      ...
      <ContextMenuContent className="w-48">
        <ContextMenuItem onClick={(event) => onReply(event, item.id)}>
          <Reply className="mr-2 h-4 w-4" />
          Reply
        </ContextMenuItem>
        <ContextMenuItem onClick={(event) => onArchive(event, item.id)}>
          <Archive className="mr-2 h-4 w-4" />
          Archive
        </ContextMenuItem>
        <ContextMenuItem onClick={(event) => onOpenLabelDialog(event, item.id)}>
          <Tag className="mr-2 h-4 w-4" />
          Add Label
        </ContextMenuItem>
        <ContextMenuItem onClick={(event) => onTrash(event, item.id)}>
          <Trash2 className="mr-2 h-4 w-4" />
          Move to Trash
        </ContextMenuItem>
        <ContextMenuSeparator />
        <ContextMenuItem onClick={(event) => onToggleRead(event, item.id)}>
...
export function MailList({ items, onReply }: MailListProps) {
  ...
  const handleReply = (event: PropagationEvent, mailId: string) => {
  ...
  const handleArchive = (event: PropagationEvent, mailId: string) => {
    event.stopPropagation()
    archiveMail(mailId)
  }

  const handleOpenLabelDialog = (event: PropagationEvent, mailId: string) => {
    event.stopPropagation()
    setSelectedMailForLabel(mailId)
    setLabelDialogOpen(true)
  }

  const handleTrash = (event: PropagationEvent, mailId: string) => {
    event.stopPropagation()
    deleteMail(mailId)
  }

  const handleToggleRead = (event: PropagationEvent, mailId: string) => {
    event.stopPropagation()
    toggleRead(mailId)
  }
