...
export interface BackgroundSyncOptions {
  ...
  onSyncComplete?: (data: BackgroundSyncResponse) => void
...
export function useBackgroundSync(options: BackgroundSyncOptions = {}) {
  ...
  const performSync = useCallback(async () => {
    ...
    try {
      ...
      const data = await response.json() as BackgroundSyncResponse
      ...
      if (data.success) {
        ...
        if ((data.totalNewEmails || 0) > 0 && 'Notification' in window && Notification.permission === 'granted') {
          const preferencesResponse = await fetch('/api/mail/preferences')
          const preferences = await preferencesResponse.json() as PreferencesResponse
          if (preferences.preferences?.desktopNotifications) {
            new Notification('New mail in gyattmail', { body: `${data.totalNewEmails} new message${data.totalNewEmails === 1 ? '' : 's'}` })
          }
        }
