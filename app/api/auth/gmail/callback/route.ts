  ...
  try {
    ...
    for (const account of gmailAccounts) {
      if (account.connectionStatus === 'failed') {
        ...
        console.log(`Deleted failed Gmail account: ${account.email}`)
      }
    }

    // Re-check after deletion for accurate count
    const remainingAccounts = await getAccountsByEmail(email)
    const workingGmailAccounts = remainingAccounts.filter(
      (acc) => acc.provider === 'gmail' && acc.connectionStatus !== 'failed'
    )

    // If a working account already exists, update it with fresh tokens instead of creating duplicate
    if (workingGmailAccounts.length > 0) {
      const existingAccount = workingGmailAccounts[0]
    ...
    const accountNumber = workingGmailAccounts.length + 1
